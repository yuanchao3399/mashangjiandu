          ...
          <el-col :span="12">
            <el-form-item label="角色">
              <el-select v-model="form.roleIds" placeholder="请选择角色">
                <el-option :label="item.roleName" :value="item.roleId" :disabled="item.status == 1" v-for="item in roleOptions" :key="item.roleId">
                  {{item.roleName}}
                </el-option>
              </el-select>
            </el-form-item>
  ...
  methods: {
    ...
    handleUpdate (row) {
      ...
      getUser(userId).then(response => {
        ...
        let str = response.roleIds.toString()
        this.form.roleIds = parseInt(str)
        this.open = true;
        this.title = "修改用户";
        this.form.password = "";
      });
